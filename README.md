...
| 目錄 | 用途 | 使用原則 |
| --- | --- | --- |
| `roadmap/` | 學習路線、階段規劃、源碼地圖與進度追蹤 | 只放規劃與導航，不放完整筆記、實作程式碼或 AI prompts。 |
| `origin/` | View UI Plus 原始碼與來源材料 | 保存來源材料，不混入個人改寫。 |
| `apps/` | 可執行的元件仿寫、重構與企業封裝練習 | 用可執行元件驗證理解，不取代正式筆記。 |
| `docs/` | 正式源碼筆記 | 內容要回到源碼依據，不只整理結論。 |
| `prompts/` | AI 協作規則、格式、流程與檢查標準 | 保存 AI 協作規則，不取代筆記與實作。 |
