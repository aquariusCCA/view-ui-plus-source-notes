...
4. 檢查 AI 回報的預計正式筆記檔案、主線、對應 atomic、來源依據與需要人工確認的內容。
...
請依照草稿規則輸出處理摘要、正式筆記生成提案、建議段落結構、需要人工確認的內容、不納入正式筆記的內容，以及第二階段執行說明。
```
...
完成後請回報處理摘要、建立 / 更新清單、檢查結果，以及是否建議後續執行 notes content review、檢查 `00-roadmap/progress-tracker.md` 或檢查下游同步需求。不要直接同步 tracker。
```
...
| `我希望 AI 特別注意` | 第一階段需要加強檢查的面向 | `請特別注意 runtime 與 d.ts 型別落差` |
