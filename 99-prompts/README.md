...
`99-prompts/` 是 View UI Plus Learning Notes 的 **AI 協作規則管理區**。

它不存放正式學習內容，也不存放 View UI Plus 原始碼、章節狀態或進度規劃；它只負責管理 AI 在整理來源、閱讀源碼、生成筆記、檢查內容、設計練習與製作複習材料時應遵守的規則、流程、格式、範例與品質標準。

```text
00-roadmap/ = 學習路線、閱讀順序、資料流與進度規格
01-origin/  = View UI Plus 原始碼、官方文件、截圖與外部來源
03-19/      = 架構、插件、共用邏輯、元件、樣式、測試、建置等主題筆記
20-imitation/ = 根據正式筆記產生的仿寫練習
21-enterprise-wrappers/ = 根據正式筆記產生的企業封裝練習
22-review-and-practice/ = 根據正式筆記產生的複習題、面試題與重構練習
99-prompts/ = 控制 AI 如何產生、檢查與維護上述內容的提示詞資源
```

一句話理解：

```text
99-prompts/ 不負責保存知識內容；
99-prompts/ 負責讓 AI 穩定產生高品質的 View UI Plus 學習內容。
```
...
`99-prompts/` 的目標不是保存單次提問，而是建立一套可長期使用、可組合、可維護的 AI 協作系統。
...
`99-prompts/` 的子目錄不是和這七個元素一對一對應，而是依照 **Prompt 的維護生命週期** 拆分。
...
七個 99-prompts/ 子目錄 = 管理 Prompt 資源的資料夾架構
```
...
7. **Prompt 不取代正式筆記**：正式知識內容應放在對應章節、仿寫、企業封裝或複習目錄，不要放在 `99-prompts/`。
...
`99-prompts/` 的價值不在於把 Prompt 拆得越細越好，而在於讓 View UI Plus 的 AI 輔助學習流程更穩定、更可控、更容易維護。
