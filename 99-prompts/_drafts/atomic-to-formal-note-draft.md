...
正式筆記也必須是實戰判斷與下游練習的基準；讀者讀完後應能拿它做源碼追蹤、元件設計判斷、PR review、企業封裝取捨或實作檢查。
```
...
正式筆記必須內建實戰轉化層，不能只停留在「看過、理解過」。每篇正式筆記至少要包含以下一種實戰轉化，且優先同時包含使用場景與操作任務：

1. 實戰使用場景：說明這個知識可用在哪些工程情境，例如新增元件、排查全域服務、檢查型別落差、設計企業封裝或 review 元件庫 PR。
2. 實務判斷標準：說明遇到類似問題時要如何判斷，例如何時應該做全域註冊、何時應該補型別、何時應該回查樣式入口。
3. 操作檢查任務：列出讀者可以實際查哪些檔案、比對哪些 runtime / type / style / docs 表面、完成後應得到什麼結論。
4. 常見誤用或風險：指出哪些結論不能直接套用、哪些地方需要回到 source 或 atomic 查證。

實戰轉化必須由 atomic、origin、source 或明確推論支撐，不得為了增加實戰感而補入無來源知識。

禁止空泛寫法：

1. 只寫「可以提升維護性」但沒有具體場景。
2. 只寫「適合企業封裝」但沒有說明封裝取捨。
3. 只寫「要注意型別」但沒有指出要查哪個型別入口或可能造成什麼使用者側問題。
4. 只寫抽象心得，沒有可操作的檔案、判斷點、任務或驗收結果。
...
說明這個主題在實際開發、源碼追蹤、元件封裝、PR review 或問題排查中怎麼用。

## 實作檢查任務

列出 2 到 5 個可操作任務，例如要查哪些檔案、比對哪些 runtime / type / style 表面、完成後應能得到什麼結論。
...
問題是這句沒有說明使用場景、檢查檔案、判斷標準或可操作任務，讀者讀完後仍然只停留在「知道這很重要」。

合格寫法：

```md
如果要新增一個全域服務，先檢查 runtime 是否在 `src/index.js` 掛到 `app.config.globalProperties`，再檢查 `types/index.d.ts` 是否擴充 `ComponentCustomProperties`。如果只做 runtime 掛載，Vue 可以執行，但 TypeScript 使用者可能沒有 `$Xxx` 的型別提示。
```

合格原因：

1. 明確指出實戰場景：新增全域服務。
2. 明確指出檢查檔案：`src/index.js` 與 `types/index.d.ts`。
3. 明確指出判斷標準：runtime 掛載與 type 擴充要分開檢查。
4. 明確指出風險：runtime 可用不代表 TypeScript 使用者側體驗完整。

實戰轉化不是多寫心得，而是把來源事實轉成場景、檢查點和判斷標準。
...
6. 開頭不只說明讀完會懂什麼，也要說明讀完能拿去做什麼。
7. 每篇至少有一段把 View UI Plus 的架構事實轉成實務判斷或操作任務。
...
5. 可直接支撐 `20-imitation/` 仿寫任務的實作切入點。
6. 可直接支撐 `21-enterprise-wrappers/` 企業封裝取捨的判斷標準。
7. 可直接支撐 `22-review-and-practice/` 操作題、PR review 題與源碼追蹤題的檢查任務。
...
8. 讀者讀完這篇後，能用它完成哪個實戰任務？
9. 這篇是否提供了可操作的檔案檢查、設計判斷或封裝取捨？
10. 這篇是否避免只停留在「看過、理解過」而沒有實務輸出？
```
