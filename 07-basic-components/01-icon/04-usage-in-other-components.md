...
`Avatar` 的重點通常不是互動，而是「當不同內容來源存在時，應該顯示哪一種內容」。例如使用者可能提供圖片、文字、slot，或者只提供一個 icon。`Avatar` 在沒有圖片或文字時可以使用 `icon` / `customIcon`，這表示 `Icon` 在這裡是一種 fallback 內容。
...
某些圖標可能由父元件 props 或 `$VIEWUI` 全域設定決定。這表示閱讀時不能只看 `Icon` 的呼叫點，還要往上追設定來源。例如某個元件的 arrow icon 可能來自：
...
`customIcon`。在高階元件中，`customIcon` 通常表示父元件願意把自訂 class 傳給 `Icon` 的 `custom` prop，讓使用者接入自己的 icon font。
...
有些地方不一定透過 `Icon` component，而是直接寫：
