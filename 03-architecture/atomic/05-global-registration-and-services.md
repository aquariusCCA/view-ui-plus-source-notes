...
> 來源：
> - 03-architecture/origin/05-registration-flow.md / # 全域註冊流程
> - 03-architecture/origin/04-entry-design.md / ## 批次匯出與批次註冊
> - 03-architecture/origin/07-core-design-principles.md / ## 全域配置
...
`$VIEWUI` 把多個跨元件預設值集中在一起，例如尺寸、transfer、箭頭圖示、Modal 行為、Typography 設定等。它可以提供一致體驗，但也讓元件和全域環境產生關聯，所以閱讀元件時要注意是否有讀取 `$VIEWUI`。
